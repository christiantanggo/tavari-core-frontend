// components/VoiceAgent/AgentList.jsx
// List of voice agents with status and actions
...
const AgentList = ({ agents, businessId, onRefresh, voiceAgentService }) => {
  ...
  const handleToggleActive = async (agent) => {
    ...
    if (agent.is_active) {
      try {
        await voiceAgentService.updateAgent(agent.id, {
          is_active: false,
        });
  ...
  const handlePhoneNumberSelected = async (phoneNumberData) => {
    ...
    try {
      console.log('🚀 Activating agent:', phoneBrowserAgent.id, 'with phone:', phoneNumber);
      if (isForwarding) {
        console.log('📞 Forwarding enabled:', { forwardToPhone, ringCount });
      }
      
      toast.loading('Creating Vapi assistant...', { id: 'activating' });
      
      // Create Vapi agent with phone number
      await voiceAgentService.createVapiAgent(phoneBrowserAgent.id, phoneNumber);
      
      // If forwarding is enabled, update agent with forwarding settings
      if (isForwarding && forwardToPhone) {
        await voiceAgentService.updateAgent(phoneBrowserAgent.id, {
          forward_to_phone: forwardToPhone,
          forward_ring_count: ringCount,
          forward_enabled: true,
        });
        
        // Configure forwarding in Telnyx (if we have a purchased number)
        // This will be handled by the webhook or a separate configuration step
        toast.loading('Configuring call forwarding...', { id: 'forwarding' });
        // Note: Forwarding configuration may need to happen after number is connected
      }
  ...
  const handleDelete = async (agent) => {
    ...
    try {
      await voiceAgentService.deleteAgent(agent.id);
    ...
    return (
      <div style={{
        ...
        <p style={{
          ...
          Create your first AI voice agent to start answering calls automatically
        </p>
      </div>
    );
  }
