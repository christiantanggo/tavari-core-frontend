-- Fix appbuilder_update_module_usage function to handle missing records
-- This function should create the record if it doesn't exist, or update if it does
...
BEGIN
    -- Store parameter in local variable to avoid column/parameter ambiguity
    ...
    ON CONFLICT (business_id, module_key) DO UPDATE SET
        ...
        last_used = now(),
        updated_at = now(),
        module_name = EXCLUDED.module_name; -- Update module_name in case it changed in app_modules
END;
$$;
