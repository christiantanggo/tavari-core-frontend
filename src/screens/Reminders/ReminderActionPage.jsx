      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 12px 40px rgba(15,23,42,0.12);">
        <tr>
          <td style="background:linear-gradient(135deg,#0f766e 0%,#0d9488 100%);padding:32px 28px;text-align:center;color:#ffffff;">
            <div style="font-size: 11px;letter-spacing:0.14em;text-transform:uppercase;font-weight:700;opacity:0.92;">Tavari Reminder</div>
...
/**
 * Public landing page for email reminder action links (Complete / Remind tomorrow).
 * Delegates to reminder-action edge function via GET (returns HTML).
 */
export default function ReminderActionPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      setHtml('<p style="font-family:Arial,sans-serif;padding:40px;text-align:center;color:#b91c1c;">Invalid reminder link.</p>');
      setLoading(false);
      return;
    }

    const base = import.meta.env.VITE_SUPABASE_URL?.replace(/\/$/, '');
    const url = `${base}/functions/v1/reminder-action?token=${encodeURIComponent(token)}&action=${encodeURIComponent(action)}`;
    ...
    fetch(url, { method: 'GET', headers: { apikey: import.meta.env.VITE_SUPABASE_ANON_KEY } })
      ...
      .catch(() => {
        setHtml(
          '<p style="font-family:Arial,sans-serif;padding:40px;text-align:center;color:#b91c1c;">Could not process this reminder link. Please try again later.</p>'
        );
        setLoading(false);
      });
  }, [params]);
