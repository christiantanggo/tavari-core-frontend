-- Unify Task Manager training with HR training modules.
-- HR training items become the canonical training content; task resources can now
-- point at a HR module while still preserving legacy task-local resources.

ALTER TABLE public.task_manager_training_resources
  ADD COLUMN IF NOT EXISTS hr_training_item_id uuid REFERENCES public.hr_training_items(id) ON DELETE CASCADE;

CREATE INDEX IF NOT EXISTS idx_task_manager_training_resources_hr_item
  ON public.task_manager_training_resources (business_id, hr_training_item_id)
  WHERE hr_training_item_id IS NOT NULL;
...
BEGIN
  ...
  IF v_hr_training_item_id IS NOT NULL THEN
    ...
    SET
      status = CASE WHEN hr_training_assignments.status = 'assigned' THEN 'completed' ELSE hr_training_assignments.status END,
...
GRANT EXECUTE ON FUNCTION public.task_manager_hr_training_resource_rows(public.hr_training_items, uuid, uuid) TO anon, authenticated;
