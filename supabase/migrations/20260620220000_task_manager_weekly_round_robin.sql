-- Weekly round robin: rotating task lists that reset every Sunday (business timezone).

ALTER TABLE public.task_manager_round_robin_groups
  ADD COLUMN IF NOT EXISTS reset_cadence text NOT NULL DEFAULT 'on_complete'
    CHECK (reset_cadence IN ('on_complete', 'weekly_sunday')),
...
ALTER TABLE public.task_manager_tasks
  ...
  CHECK (due_schedule_mode IN ('specific_date', 'frequency', 'round_robin', 'weekly_round_robin'));

CREATE OR REPLACE FUNCTION public.task_manager_business_week_start(p_business_id uuid)
RETURNS date
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  -- PostgreSQL DOW: Sunday = 0
  RETURN v_business_date - EXTRACT(DOW FROM v_business_date)::integer;
END;
$$;
...
BEGIN
  ...
  FOR r IN
    ...
    WHERE g.business_id = p_business_id
      ...
      AND COALESCE(g.current_week_start, '1900-01-01'::date) < v_week_start
  LOOP
    ...
    SET
      ...
      available_at = now(),
      due_at = NULL
    WHERE round_robin_group_id = r.id
...
-- Patch complete_task: weekly round robin groups do not restart when all tasks finish mid-week.
