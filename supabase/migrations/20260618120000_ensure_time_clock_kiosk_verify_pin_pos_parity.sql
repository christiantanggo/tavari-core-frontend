-- Re-apply POS-aligned PIN verification for kiosk RPCs (clock_in, clock_out, etc.).
-- If production skipped earlier timestamped migrations, verify_pin could omit user_roles
-- while get_all_staff_pins_for_unlock (browser fallback) still matches — then clock_in
-- raises "Invalid PIN" because it only trusts time_clock_kiosk_verify_pin.
