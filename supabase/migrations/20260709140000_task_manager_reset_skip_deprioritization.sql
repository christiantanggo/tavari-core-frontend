-- Skipped tasks are "for now" only. Reset deprioritization so recent skips come back into rotation.

CREATE OR REPLACE FUNCTION public.task_manager_reset_skip_deprioritization(p_business_id uuid)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WITH reset_rows AS (
    ...
    WHERE tm.business_id = p_business_id
      ...
      AND tm.last_skipped_at IS NOT NULL
      AND (
        tm.last_skipped_at < v_today_start
        OR tm.last_skipped_at < now() - interval '48 hours'
      )
    RETURNING tm.id
  )
  SELECT count(*)::integer INTO v_reset FROM reset_rows;

  RETURN v_reset;
END;
$$;

-- Bring tasks skipped in the last few days back into normal queue priority now.
-- Keeps skip_count / activity history; only clears temporary deprioritization.
UPDATE public.task_manager_tasks
SET
  last_skipped_at = NULL,
  skip_queue_stamp = 0
WHERE status IN ('to_do', 'in_progress', 'blocked')
  AND last_skipped_at IS NOT NULL
  AND last_skipped_at >= now() - interval '3 days';
...
BEGIN
  ...
  PERFORM public.task_manager_reset_skip_deprioritization(p_business_id);
...
GRANT EXECUTE ON FUNCTION public.task_manager_reset_skip_deprioritization(uuid) TO anon, authenticated;

-- Patch queue preview to apply the same daily skip reset.
