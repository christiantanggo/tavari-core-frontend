-- Storage for forms escalation photos/videos (kiosk + dashboard uploads).

INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES (
  'forms-escalation-media',
  'forms-escalation-media',
  true,
  104857600,
  ARRAY[
...
DROP POLICY IF EXISTS "Forms escalation media public read" ON storage.objects;
CREATE POLICY "Forms escalation media public read"
ON storage.objects
FOR SELECT
TO anon, authenticated
USING (bucket_id = 'forms-escalation-media');

DROP POLICY IF EXISTS "Forms escalation media anon upload" ON storage.objects;
CREATE POLICY "Forms escalation media anon upload"
ON storage.objects
FOR INSERT
TO anon, authenticated
WITH CHECK (bucket_id = 'forms-escalation-media');

-- Attach escalation follow-up details from p_responses._escalations to alert payloads.
