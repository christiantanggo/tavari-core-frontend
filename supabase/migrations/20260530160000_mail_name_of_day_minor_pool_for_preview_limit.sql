-- Mail automation preview: cap matching minors so very common names cannot exhaust statement timeout.
...
AS $$
  ...
  ) AS _nod_preview_union
  LIMIT 4000;
$$;

COMMENT ON FUNCTION public.mail_name_of_day_minor_pool_for_preview(uuid, text, text) IS
  'Name-of-Day automation preview: minors matching girl/boy normalized names (max 4000 rows).';
