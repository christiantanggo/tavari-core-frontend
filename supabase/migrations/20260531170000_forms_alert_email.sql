-- Email dispatch for Tavari Forms out-of-range alerts.

CREATE EXTENSION IF NOT EXISTS pg_net;

INSERT INTO public.system_runtime_secrets (key_name, secret_value)
VALUES (
  'forms_alert_email_secret',
  replace(gen_random_uuid()::text, '-', '') || replace(gen_random_uuid()::text, '-', '')
)
ON CONFLICT (key_name) DO NOTHING;

CREATE OR REPLACE FUNCTION public.forms_trigger_alert_email(p_alert_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT secret_value INTO dispatch_secret
  FROM public.system_runtime_secrets
  WHERE key_name = 'forms_alert_email_secret';
  ...
  PERFORM net.http_post(
    url := project_url || '/functions/v1/forms-send-alert-email',
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'x-forms-alert-email-secret', dispatch_secret
    ),
    body := jsonb_build_object('alertId', p_alert_id),
    timeout_milliseconds := 60000
  );
END;
$$;

REVOKE ALL ON FUNCTION public.forms_trigger_alert_email(uuid) FROM PUBLIC;
...
COMMENT ON FUNCTION public.forms_trigger_alert_email IS
  'Async POST to forms-send-alert-email when a forms manager alert is created.';
