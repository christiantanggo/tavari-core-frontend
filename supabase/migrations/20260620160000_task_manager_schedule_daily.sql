-- Allow daily frequency on task_manager_tasks.schedule_type

ALTER TABLE public.task_manager_tasks
  DROP CONSTRAINT IF EXISTS task_manager_tasks_schedule_type_check;

ALTER TABLE public.task_manager_tasks
  ADD CONSTRAINT task_manager_tasks_schedule_type_check
  CHECK (schedule_type IS NULL OR schedule_type IN (
    'once', 'daily', 'weekly', 'biweekly', 'monthly', 'monthly_weekday'
  ));

-- Re-open daily frequency tasks +1 day after completion
