-- Per-campaign rolling 7 local days (sent/open/click rates) ending on digest anchor date.
  ...
  FROM (
    ...
    FROM (
      ...
      LEFT JOIN (
        SELECT
          mc.id AS campaign_id,
          (mcs.sent_at AT TIME ZONE p_timezone)::date AS day_local,
          COUNT(*) FILTER (
            WHERE mcs.status = 'sent'
              AND mcs.sent_at IS NOT NULL
          ) AS sent,
        ...
        WHERE (mcs.sent_at AT TIME ZONE 'UTC') >= (v_anchor_end - 6)::timestamp AT TIME ZONE p_timezone
          AND (mcs.sent_at AT TIME ZONE 'UTC') < (v_anchor_end + 1)::timestamp AT TIME ZONE p_timezone
        GROUP BY mc.id, (mcs.sent_at AT TIME ZONE p_timezone)::date
...
COMMENT ON FUNCTION public.mail_daily_digest_stats(uuid, timestamptz, timestamptz, text) IS
  'Digest stats: summary uses p_range_* window; campaigns_seven_day is per-campaign rolling 7 local days '
  'ending on anchor_local_date (same as aggregate last_seven_days).';
