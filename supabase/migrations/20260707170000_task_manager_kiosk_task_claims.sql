-- Kiosk task claims: one active facility task per employee; second kiosk gets next available task.
...
CREATE OR REPLACE FUNCTION public.task_manager_kiosk_task_claim_available(
  p_claimed_by uuid,
  p_status text,
  p_handoff_at timestamptz,
...
AS $$
BEGIN
  IF p_employee_id IS NOT NULL AND p_claimed_by = p_employee_id THEN
    RETURN true;
  END IF;

  IF p_claimed_by IS NOT NULL
     AND p_employee_id IS NOT NULL
     AND p_claimed_by <> p_employee_id
     AND p_status = 'in_progress'
     AND p_handoff_at IS NULL
     AND (
       p_claimed_at IS NULL
       OR p_claimed_at > now() - (GREATEST(COALESCE(p_stale_minutes, 45), 1) || ' minutes')::interval
     )
  THEN
    RETURN false;
  END IF;

  IF p_status = 'in_progress' AND p_claimed_by IS NULL AND p_handoff_at IS NOT NULL THEN
    RETURN true;
  END IF;

  IF p_status = 'to_do' AND p_claimed_by IS NULL THEN
    RETURN true;
  END IF;
...
CREATE OR REPLACE FUNCTION public.task_manager_release_stale_task_claims(
  p_business_id uuid,
  p_stale_minutes integer DEFAULT 45
)
RETURNS integer
...
BEGIN
  WITH released AS (
    ...
    WHERE tm.business_id = p_business_id
      ...
      AND tm.handoff_at IS NULL
      AND tm.claimed_at IS NOT NULL
      AND tm.claimed_at < now() - (GREATEST(COALESCE(p_stale_minutes, 45), 1) || ' minutes')::interval
    RETURNING tm.id, tm.claimed_by
  )
  ...
  SELECT
    ...
    r.claimed_by,
    'task_claim_released',
    jsonb_build_object('reason', 'stale_timeout', 'stale_minutes', p_stale_minutes)
  FROM released r;
...
CREATE OR REPLACE FUNCTION public.task_manager_release_facility_task_claim(
  p_business_id uuid,
  p_task_id uuid,
  p_employee_id uuid,
...
BEGIN
  ...
  IF v_task.id IS NULL OR v_task.status NOT IN ('to_do', 'in_progress') THEN
    RETURN jsonb_build_object('success', true, 'released', false);
  END IF;

  IF v_task.claimed_by IS DISTINCT FROM p_employee_id THEN
    RETURN jsonb_build_object('success', true, 'released', false);
  END IF;
  ...
  VALUES (
    ...
    p_employee_id,
    'task_claim_released',
    jsonb_build_object('reason', 'employee_session_end')
  );
...
CREATE OR REPLACE FUNCTION public.task_manager_touch_facility_task_claim(
  p_business_id uuid,
  p_task_id uuid,
  p_employee_id uuid,
  p_pin text
...
CREATE OR REPLACE FUNCTION public.task_manager_facility_task_payload(
  p_business_id uuid,
  p_employee_id uuid,
  p_task_id uuid,
  p_busy_signals jsonb
)
RETURNS TABLE (
  task_id uuid,
  template_id uuid,
...
AS $$
  SELECT
    ...
    c.category,
    c.category_id,
    COALESCE(cat.location_sensitivity, 'none'),
    c.priority,
    c.assignment_scope,
    c.due_at,
    c.requires_photo,
    c.photo_requirement_mode,
    c.requires_notes,
    c.instructions,
    c.checklist,
    COALESCE((
    ...
    p_busy_signals
  FROM public.task_manager_tasks c
  LEFT JOIN public.task_manager_categories cat ON cat.id = c.category_id
...
GRANT EXECUTE ON FUNCTION public.task_manager_release_facility_task_claim(uuid, uuid, uuid, text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.task_manager_touch_facility_task_claim(uuid, uuid, uuid, text) TO anon, authenticated;
