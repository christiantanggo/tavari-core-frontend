-- Per rolling local day: add clicked count + click_rate_pct alongside open metrics (mail_daily_digest_stats.last_seven_days).
...
COMMENT ON FUNCTION public.mail_daily_digest_stats(uuid, timestamptz, timestamptz, text) IS
  ...
  'Rolling last_seven_days rows include open_rate_pct and click_rate_pct per local calendar day (sent_at-based). '
