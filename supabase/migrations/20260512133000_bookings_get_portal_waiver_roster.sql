-- Booking portal: full participant roster for a loyalty customer (bypasses anon RLS on nested waiver_participants).
-- Used by CustomerPortalActivityPage so "Select Participants" matches data the customer portal can see via service-role APIs.
...
BEGIN
  ...
  RETURN QUERY
  -- Primary signer row per waiver (stable id = waiver_signatures.id for portal UI)
  SELECT
    ws.id,
    ws.id AS waiver_id,
    ws.first_name,
  ...
  -- Minors / additional adults stored on waiver_participants for those waivers
  SELECT
    wp.id,
    wp.waiver_id,
    wp.first_name,
    ...
    COALESCE(wp.is_account_owner, false) AS is_account_owner,
  ...
  -- Portal-created rows on waiver_participants (often waiver_id IS NULL until linked to a waiver)
  SELECT
    wp.id,
    wp.waiver_id,
    wp.first_name,
  ...
  -- Legacy / parallel booking_customer_participants rows (used for participant_id on bookings)
  SELECT
    bcp.id,
...
COMMENT ON FUNCTION public.bookings_get_portal_waiver_roster(uuid, uuid) IS
  'Definer read: all booking/waiver people for a portal customer (primaries from waiver_signatures, minors/additional on those waivers, orphan waiver_participants, booking_customer_participants).';
