-- Allow training documents to be attached directly to one-off task instances.

ALTER TABLE public.task_manager_training_resources
  ADD COLUMN IF NOT EXISTS task_id uuid REFERENCES public.task_manager_tasks(id) ON DELETE CASCADE;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'task_manager_training_resources_owner_check'
      AND conrelid = 'public.task_manager_training_resources'::regclass
  ) THEN
    ALTER TABLE public.task_manager_training_resources
      ADD CONSTRAINT task_manager_training_resources_owner_check
      CHECK (template_id IS NOT NULL OR task_id IS NOT NULL);
  END IF;
END;
$$;

CREATE INDEX IF NOT EXISTS idx_task_manager_training_resources_task
  ON public.task_manager_training_resources (business_id, task_id, created_at);
...
BEGIN
  ...
  SELECT
    ...
    COALESCE((
      ...
      ) ORDER BY CASE WHEN tr.task_id = c.id THEN 0 ELSE 1 END, tr.created_at)
    ...
    COALESCE((
      ...
      LEFT JOIN public.task_manager_employee_training_completions etc
        ON etc.resource_id = tr.id AND etc.employee_id = p_employee_id
      WHERE (tr.task_id = c.id OR (c.template_id IS NOT NULL AND tr.template_id = c.template_id AND tr.task_id IS NULL))
...
BEGIN
  ...
  IF v_task.due_schedule_mode = 'frequency' AND v_task.schedule_type IS NOT NULL THEN
    ...
    ELSE
      ...
      IF v_task.schedule_type = 'daily' THEN
