-- Fix 23502: kiosk atomic submit omitted waiver_participants.account_card_style / restrict_check_in;
-- jsonb_populate_record leaves them NULL and INSERT ... VALUES (row.*) bypasses table DEFAULTs.
...
BEGIN
  ...
  FOR part IN SELECT value FROM jsonb_array_elements(v_parts) AS e (value) LOOP
    ...
    -- jsonb_populate_record omits keys as NULL; INSERT ... VALUES (row.*) bypasses table DEFAULTs.
    v_part_row.account_card_style := coalesce(v_part_row.account_card_style, 'default');
    v_part_row.restrict_check_in := coalesce(v_part_row.restrict_check_in, false);
