-- mail_daily_digest_stats: per-campaign click counts/rates; rollout batch slice for segment-style comparison.
...
COMMENT ON FUNCTION public.mail_daily_digest_stats(uuid, timestamptz, timestamptz, text) IS
  'Aggregates mail stats for a UTC window. Campaign rows include open/click rates per campaign; '
  'rollout_batches compares staged rollout slices when sends reference rollout_batch_id. '
  'Rates use sent-status rows as denominator (one row per recipient delivery).';
