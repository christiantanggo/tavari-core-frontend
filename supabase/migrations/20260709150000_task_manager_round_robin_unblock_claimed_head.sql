-- Round-robin head: don't let one actively-claimed in_progress task block the whole group.
    ...
    ORDER BY
      ...
      CASE
        WHEN tm.status = 'in_progress'
          ...
          AND tm.claimed_at > now() - (v_stale_minutes || ' minutes')::interval
        THEN 1
        ELSE 0
      END ASC,
...
-- Patch queue preview round-robin lateral pick to match.
...
BEGIN
  ...
  LOOP
    ...
    LEFT JOIN LATERAL (
      ...
      ORDER BY
        CASE
          WHEN h.status = 'in_progress'
            AND h.claimed_by IS NOT NULL
            AND h.claimed_at IS NOT NULL
            AND h.claimed_at > now() - (v_stale_minutes || ' minutes')::interval
          THEN 1
          ELSE 0
        END ASC,
