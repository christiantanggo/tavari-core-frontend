...
Deno.serve(async (req) => {
  ...
  if (!creds) {
    return jsonResponse({ error: "Authorize.net credentials not configured for this business" }, 400);
  }
  ...
  try {
    ...
    for (const summary of summaries) {
      ...
      const result = await upsertAuthorizeNetTransactionRow(admin, {
        businessId,
        details,
    ...
    console.error("[authorize-net-sync-sales]", e);
