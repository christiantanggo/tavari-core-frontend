...
import {
  loadPublicDigitalSignageCreatives,
  parseDigitalSignageAudience,
} from "../_shared/tavariPublicDigitalSignage.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (moduleErr) {
      console.error("[tavari-api-digital-signage] module check", moduleErr);
    ...
    const audience = parseDigitalSignageAudience(
      String(body.audience ?? url.searchParams.get("audience") ?? "venue"),
    );
    ...
    const payload = await loadPublicDigitalSignageCreatives(supabase, businessId, {
      audience,
      activeOnly,
    ...
    return json({
      ...payload,
      localDate: payload.calendarDate,
    } as unknown as Record<string, unknown>);
  } catch (error) {
    console.error("[tavari-api-digital-signage]", error);
    return json({
      ok: false,
