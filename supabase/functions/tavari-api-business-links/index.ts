...
serve(async (req) => {
  ...
  try {
    ...
    if (moduleErr) {
      console.error("[tavari-api-business-links] module check", moduleErr);
    ...
    const payload = await loadPublicBusinessLinks(supabase, businessId, {
      siteUrl: Deno.env.get("PUBLIC_SITE_URL") || undefined,
    });

    return json(payload as unknown as Record<string, unknown>);
  } catch (error) {
    console.error("[tavari-api-business-links]", error);
    return json({
      ok: false,
