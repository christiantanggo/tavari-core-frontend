// Public read-only business name (same field as Dashboard → Settings → Basic Information).
...
serve(async (req) => {
  ...
  try {
    ...
    if (moduleErr) {
      console.error("[tavari-api-business-name] module check", moduleErr);
    ...
    if (bizErr) {
      console.error("[tavari-api-business-name] business lookup", bizErr);
    ...
    console.error("[tavari-api-business-name]", e);
    return jsonResponse(
