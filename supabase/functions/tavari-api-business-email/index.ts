// Public read-only business email (same field as Dashboard → Settings → Basic Information).
...
serve(async (req) => {
  ...
  try {
    ...
    if (moduleErr) {
      console.error("[tavari-api-business-email] module check", moduleErr);
    ...
    if (bizErr) {
      console.error("[tavari-api-business-email] business lookup", bizErr);
    ...
    return jsonResponse({
      ...
      email: typeof business.business_email === "string" ? business.business_email.trim() : "",
    });
  } catch (e) {
    console.error("[tavari-api-business-email]", e);
    return jsonResponse(
