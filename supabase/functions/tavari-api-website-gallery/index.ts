...
import { loadPublicWebsiteGallery } from "../_shared/tavariPublicWebsiteGallery.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (moduleErr) {
      console.error("[tavari-api-website-gallery] module check", moduleErr);
    ...
    const payload = await loadPublicWebsiteGallery(supabase, businessId, {
      activeOnly,
      tag: tag || undefined,
    ...
    console.error("[tavari-api-website-gallery]", error);
    return json({
      ok: false,
