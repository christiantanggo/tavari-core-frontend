// Public read-only formatted business address (Dashboard → Settings → Basic Information).
...
/** e.g. "539 First St, London, ON N5V 1Z5" */
...
serve(async (req) => {
  ...
  try {
    ...
    if (moduleErr) {
      console.error("[tavari-api-business-address] module check", moduleErr);
    ...
    const { data: business, error: bizErr } = await supabase
      .from("businesses")
      .select("id, business_address, business_city, business_state, business_postal")
      .eq("id", businessId)
      .maybeSingle();

    if (bizErr) {
      console.error("[tavari-api-business-address] business lookup", bizErr);
    ...
    const address = formatBusinessAddress(street, city, state, postalCode);
    ...
    console.error("[tavari-api-business-address]", e);
    return jsonResponse(
