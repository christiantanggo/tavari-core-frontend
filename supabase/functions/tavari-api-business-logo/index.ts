// Public read-only business logo (Dashboard → Settings → Basic Information → Logo, app_branding.logo_url).
...
serve(async (req) => {
  ...
  try {
    ...
    if (moduleErr) {
      console.error("[tavari-api-business-logo] module check", moduleErr);
    ...
    if (bizErr) {
      console.error("[tavari-api-business-logo] business lookup", bizErr);
    ...
    if (brandingErr) {
      console.error("[tavari-api-business-logo] branding lookup", brandingErr);
      return jsonResponse({ ok: false, error: "Could not load business logo." }, 500);
    }
    ...
    console.error("[tavari-api-business-logo]", e);
    return jsonResponse(
