...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'list') {
      ...
      return json({ ok: true, employee, business, updates: data || [] })
    }

    if (action === 'acknowledge') {
      const updateId = String(body.update_id || '')
      if (!updateId) return json({ error: 'Missing update_id' }, 400)
      ...
      if (!existing || existing.cancelled_at) return json({ error: 'Update not found' }, 404)
      if (!existing.requires_acknowledgement) return json({ error: 'This update does not require acknowledgement' }, 400)
      if (existing.acknowledged_at) return json({ ok: true, update: existing })
    ...
    console.error('[employee-updates-action] error', error)
