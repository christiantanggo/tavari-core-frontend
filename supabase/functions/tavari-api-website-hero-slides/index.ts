...
import { loadPublicWebsiteHeroSlides } from "../_shared/tavariPublicWebsiteHeroSlides.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (moduleErr) {
      console.error("[tavari-api-website-hero-slides] module check", moduleErr);
    ...
    const payload = await loadPublicWebsiteHeroSlides(supabase, businessId, {
      activeOnly,
    ...
    console.error("[tavari-api-website-hero-slides]", error);
    return json({
      ok: false,
