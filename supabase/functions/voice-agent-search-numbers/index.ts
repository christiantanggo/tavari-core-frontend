// supabase/functions/voice-agent-search-numbers/index.ts
// Search available phone numbers from Telnyx API
...
serve(async (req) => {
  ...
  try {
    ...
    // Build Telnyx API request for available numbers
    // Telnyx uses filter parameters like filter[nxx] for area code, filter[administrative_area] for state
    const params = new URLSearchParams();
    params.append('filter[country_code]', countryCode);
    params.append('page[size]', limit.toString());

    if (areaCode && areaCode.length === 3) {
      // Search by area code (NXX pattern)
    ...
    // Format numbers for display (limit to 5)
    const formattedNumbers = availableNumbers.slice(0, limit).map((num: any) => ({
      ...
      cost: {
        monthly: '15.00', // Estimated monthly cost (USD)
        setup: '0.00', // Usually free
      },
    }));
